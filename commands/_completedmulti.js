...
let optionss= User.getProperty("multidep");
let amount=options.amount
let cujren=optionss.currency
let mamounts = optionss.amount;
let mamount=mamounts*1
Libs.ResourcesLib.anotherUserRes("stdepo", 452080432).add(amount)
var json_i={
user_info:user,
amount:amount,
full_deposit_info:optionss,
date: new Date()
}
...
let wallet = optionss.address;
let currency = optionss.currency;
let cur = Bot.getProperty("cur")
let hag =User.setProperty("Reffer")
let fiat_amount = optionss.fiat_amount;
let fiat_currency = optionss.fiat_coin;
let fee = optionss.fee;
let R = Bot.getProperty("rid"+user.telegramid)
let gg = Bot.getProperty("cnoti"+R)
let txn_id = optionss.txn_id
...
Bot.run( {
    command:'/adc'+' '+peh1+' '+relg75.value()+' '+amount+' '+optionss.currency+' '+'dep1',
...
Bot.run( {
    command:'/adc'+' '+peh2+' '+relg75.value()+' '+amount+' '+optionss.currency+' '+'dep2',
...
Bot.run( {
    command:'/adc'+' '+peh3+' '+relg75.value()+' '+amount+' '+optionss.currency+' '+'dep3',
...
if(admn){
var txt8="<b>➕ New Investment Notification</b>"+'\n'+'<a href="'+'tg://user?id='+user.telegramid+'">'+user.first_name+'</a>'+' | '+'<code>'+user.telegramid+'</code> | '+' just invested <code>'+amount+' '+cur+' ('+mamount+' '+cujren+')</code> '
...
let withdrawal_text=Bot.getProperty("multi_deposit_channel_text")
if(!withdrawal_text){
var fgp="➕<b> New Deposit </b>\n\n👤 By User: "+user.first_name+" \n💰 Amount: <b>"+amount+" "+cur+" ~ "+gooo+" $</b>"+"\n💱 Currency Deposit: <b>("+mamount+" "+cujren+")</b>\n\n🖇 <b>Transaction Hash:</b>\n"+txurl2
}else{
