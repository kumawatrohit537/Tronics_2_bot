...
var depr = Bot.getProperty("dgb"+user.telegramid)
if (depr == undefined) {
  Bot.sendMessage("✊Generating a address for you")
  HTTP.get({
url:
    "https://cryptopay.up.railway.app/",
  success: "sucsdgb"})
} else {
 var wallset = Bot.getProperty("walldgb"+user.telegramid)
...
var ty = [{ title: "🔁 Refresh", command: "/refreshdgb" }]
Bot.sendMessage("⚠️ _If you send less than 8 dgb, your deposit will be cancelled!_\n\n_You can also check Your deposit by refresh button_ \n✅* Send the amount you want to invest to the following address:*")
