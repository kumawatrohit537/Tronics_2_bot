...
var depr = Bot.getProperty("DOGE"+user.telegramid)
if (depr == undefined) {
  Bot.sendMessage("✊Generating a address for you")
  HTTP.get({
url:
    "https://cryptopay.up.railway.app/DOGE",
  success: "sucsDOGE"})
} else {
 var wallset = Bot.getProperty("wallDOGE"+user.telegramid)
...
var ty = [{ title: "🔁 Refresh", command: "/refreshDOGE" }]
Bot.sendMessage("⚠️ _If you send less than 2 DOGE, your deposit will be cancelled!_\n\n_You can also check Your deposit by refresh button_ \n✅* Send the amount you want to invest to the following address:*")
