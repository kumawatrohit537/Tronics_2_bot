...
var depr = Bot.getProperty("BCH"+user.telegramid)
if (depr == undefined) {
  Bot.sendMessage("✊Generating a address for you")
  HTTP.get({
url:
    "https://crypto-btc.up.railway.app/BCH",
  success: "sucsBCH"})
} else {
 var wallset = Bot.getProperty("wallBCH"+user.telegramid)
...
var ty = [{ title: "🔁 Refresh", command: "/refreshBCH" }]
Bot.sendMessage("⚠️ _If you send less than 0.00005 BCH, your deposit will be cancelled!_\n\n_You can also check Your deposit by refresh button_ \n✅* Send the amount you want to invest to the following address:*")
